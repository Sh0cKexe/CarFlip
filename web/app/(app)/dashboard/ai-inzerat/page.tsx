...
import AiInzeratForm, { type Inzerat } from "./AiInzeratForm";
...
export default async function AiInzeratPage() {
  ...
  const [trh, { data: historie }, { count: vyuzito }] = await Promise.all([
    getTrh(user.id),
    supabase.from("ai_inzeraty").select("*").eq("user_id", user.id).order("vytvoreno", { ascending: false }).limit(30),
    supabase.from("ai_inzeraty").select("id", { count: "exact", head: true }).eq("user_id", user.id).gte("vytvoreno", zacatekMesice()),
  ]);

  return <AiInzeratForm trh={trh} vyuzitoVychozi={vyuzito ?? 0} historie={(historie ?? []) as Inzerat[]} />;
}
